...
local ContentEncoder = require('CoffeeOKGames.nakama.helpers.content_encoder')
local LibNakama = require('CoffeeOKGames.nakama.lib.nakama')
local DefoldEngineNakama = require('CoffeeOKGames.nakama.lib.engine.defold')
...
function NakamaAPI:init(server_config, local_storage)
    self.config = assert(server_config)
    self.nakama = LibNakama
    self.local_storage = local_storage
    self.content_encoder = ContentEncoder(server_config.encode_key)

    self.debug = Debug('NakamaAdapter', DEBUG)

    self.client = self:_init_client(self.config.config_client, DefoldEngineNakama)
    self:_load_state()
end
